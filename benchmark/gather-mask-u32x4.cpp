...
[[gnu::noinline]] Vector<Val, size> op0(std::span<const Val> data, Mask<32, size> m,
                                        Vector<Idx, size> idxs) {
  return {
    m[0] ? data[idxs[0]] : Val{},
    m[1] ? data[idxs[1]] : Val{},
    m[2] ? data[idxs[2]] : Val{},
...
[[gnu::noinline]] Vector<Val, size> op1(std::span<const Val> data, Mask<32, size> m,
                                        Vector<Idx, size> idxs) {
  constexpr Idx limit = Idx{1} << 31;
  if (data.size() >= limit) {
    return _mm_mask_i32gather_ps(_mm_setzero_ps(), data.data() + limit, idxs ^ limit, m, 4);
  }
  return _mm_mask_i32gather_ps(_mm_setzero_ps(), data.data(), idxs, m, 4);
}
[[gnu::noinline]] Vector<Val, size> op2(std::span<const Val> data, Mask<32, size> m,
                                        Vector<Idx, size> idxs) {
  return _mm256_mask_i64gather_ps(_mm_setzero_ps(), data.data(), _mm256_cvtepu32_epi64(idxs), m, 4);
}
...
#pragma omp parallel for default(shared) private(vdist, rng) schedule(guided)
  ...
  for (auto _ : state) {
    ...
    benchmark::DoNotOptimize(
      op(std::span{data.get(), stado::horizontal_max(idxs) + 1},
         Mask<32, size>{idist(rng) * 2 < data_size, idist(rng) * 2 < data_size,
                        idist(rng) * 2 < data_size, idist(rng) * 2 < data_size},
         idxs));
    i = (i + 4) % nidx;
  }
}
