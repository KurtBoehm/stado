...
namespace stado {
inline f32x4 lookup_part(const u32x4 index, const f32* table, std::size_t n) {
...
inline f64x2 lookup_part(const u64x2 index, const f64* table, std::size_t n) {
#if STADO_INSTRUCTION_SET >= STADO_AVX2
...
#if STADO_INSTRUCTION_SET >= STADO_AVX2
inline f32x8 lookup_part(const u32x8 index, const f32* table, std::size_t n) {
  const auto mask = part_mask<Mask<32, 8>>(n);
  return lookup_masked(mask, index, table);
}

inline f64x4 lookup_part(const u32x4 index, const f64* table, std::size_t n) {
  const auto mask = part_mask<Mask<64, 4>>(n);
  return lookup_masked(mask, index, table);
}

inline f64x4 lookup_part(const u64x4 index, const f64* table, std::size_t n) {
  const auto mask = part_mask<Mask<64, 4>>(n);
  return lookup_masked(mask, index, table);
}
#endif
...
inline f32x16 lookup_part(const u32x16 index, const f32* table, std::size_t n) {
  return _mm512_mask_i32gather_ps(_mm512_setzero_ps(), __mmask16((1U << n) - 1), index, table, 4);
}

inline f64x8 lookup_part(const u32x8 index, const f64* table, std::size_t n) {
  return _mm512_mask_i32gather_pd(_mm512_setzero_pd(), __mmask8((1U << n) - 1), index, table, 8);
}

inline f64x8 lookup_part(const u64x8 index, const f64* table, std::size_t n) {
